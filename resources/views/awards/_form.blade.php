...
<div class="w-full max-w-sm md:h-fit p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div>
            ...
            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" value="{{$award->name}}"/>
        ...
        <div class="mt-4">
            ...
            <input type="date" id="start" name="start" class="block mt-1 w-full" required value="{{$award->start}}"/>
            <x-input-error :messages="$errors->get('start')" class="mt-2" />
        </div>
        
        <div class="mt-4">
            <x-input-label for="end" :value="__('End Date')" />
            <input type="date" id="end" name="end" class="block mt-1 w-full" required value="{{$award->end}}"/>
        ...
        <div class="flex justify-between items-end mt-4">
            ...
            <div class="py-2">
                <a class="underline text-align-bottom text-sm text-gray-600 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" href="{{ route('awards.index') }}">
                    {{ __('Cancel') }}
                </a>
            </div>
        </div>
